...
public class BakeOff1 extends PApplet {
	...
	public void draw() {
		...
		text("See how fast and accurately you can click these 16 buttons.", 350, 70);
		text("A hit will leave the button green, while a miss will leave it red!", 350, 100);
	...
	public void drawButton(int i) {
        Rectangle bounds = getButtonLocation(i);
        int nextTrial = trialNum+1;
        if (nextTrial < trials.size() && trials.get(nextTrial) == i)
            fill(0,255,255, 30);
	...
	public void randomizeSquares(int i) {
	    ...
	    if (buttonStates[i] == 1) { // If hit
	        fill(0, 255, 0); // Set color to green
	    } else if (buttonStates[i] == 2) { // If miss
	        fill(255, 0, 0); // Set color to red
	    } else if (trials.get(trialNum) == i) { // Current target
	        fill(0, 0, 255); // If so, fill blue
	    } else {
	        fill(200); // If not, fill gray
	    }
