...
int main(int argc, char** argv)
{
  ...
  registers[6] = 1024;  // %esp initially set to 1024
...
instruction_t* decode_instructions(unsigned int* bytes, unsigned int num_instructions)
{
  ...
  //allocate memory for the instruction_t
 
  int i;
  for(i = 0; i < (int)num_instructions; i++){
    unsigned int raw_inst = bytes[i]; //get the raw 4-byte instruction
    
    retval[i].opcode  = (raw_inst >> 27) & 0x1F;         //opcode    (31-27) 5 bits
    retval[i].first_register  = (raw_inst >> 22) & 0x1F; //reg1      (26-22) 5 bits
    retval[i].second_register  = (raw_inst >> 17) & 0x1F;//reg2      (21-17) 5 bits
    retval[i].immediate  = (int16_t)(raw_inst & 0xFFFF); //immediate (15-0) 16 bits
...
unsigned int execute_instruction(unsigned int program_counter, instruction_t* instructions, int* registers, unsigned char* memory)
{
  ...
  case cmpl: // opcode 9
    {     
      int result = registers[instr.second_register] - registers[instr.first_register];
      // Calculate flags
      int ZF = (result == 0);
      int SF = ((result >> 31) & 1); //shift result to the least significant bit position and isolates this bit to determine if it's set
      int CF = ((unsigned int)registers[instr.second_register] < (unsigned int)registers[instr.first_register]);
      //OF is set if the result of the comparison does not have the same sign as expected. based on the operands' signs, indicating an overflow occurred. 
      int OF = ((registers[instr.second_register] < registers[instr.first_register]) != (result < 0));
      // Store flags in %eflags register (register 16)
      registers[16] = (ZF << 6) | (SF << 7) | (CF << 0) | (OF << 11);  // ZF-64 SF-128 CF-1 OF-2048;
    }
    break;

  case je:  // opcode 10
    if ((registers[16] >> 6) & 1) {  // ZF is set
      return program_counter + instr.immediate + 4;
      }  
    break;

  case jl:  // opcode 11
    if (((registers[16] >> 7) & 1) ^ ((registers[16] >> 11) & 1)) {  // SF != OF
      return program_counter + instr.immediate + 4;
    }
    break;

  case jle:  // opcode 12
    if ((((registers[16] >> 7) & 1) ^ ((registers[16] >> 11) & 1)) || ((registers[16] >> 6) & 1)) {  // (SF != OF) || ZF
      return program_counter + instr.immediate + 4;
    }
    break;
  ...
  case jbe:  // opcode 14
    if (((registers[16] >> 0) & 1) || ((registers[16] >> 6) & 1)) {  // CF || ZF
      return program_counter + instr.immediate + 4;
    }
    break;
  ...
  case call:  // opcode 16
    registers[6] -= 4;  // %esp -= 4
    *(int*)&memory[registers[6]] = program_counter + 4;  // push return address onto stack
    return program_counter + instr.immediate + 4;  // jump to target
      
  case ret:  // opcode 17
    if (registers[6] == 1024) {
      exit(0);  // exit simulation if stack pointer is at initial position
    } else {
      program_counter = *(int*)&memory[registers[6]];  // pop the return address from stack
      registers[6] += 4;  // %esp += 4
