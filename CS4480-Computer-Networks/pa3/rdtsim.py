...
class EntityA:
    ...
    def output(self, message):
        pkt = Pkt(self.make_seqnum, 0, 0, message.data)
        pkt.checksum = self.make_seqnum + int.from_bytes(pkt.payload, byteorder='little')
        self.l.append(pkt)
        self.make_seqnum = (self.make_seqnum + 1) % self.seqnum_limit

        while self.next_seqnum < self.base + self.window_size and (self.next_seqnum < len(self.l)):
            to_layer3(self, self.l[self.next_seqnum])
            #print("A: sending from output: ", self.l[self.next_seqnum])
            self.next_seqnum = self.next_seqnum + 1
            #print("A: next seqnum number from output:", self.next_seqnum)
    ...
    def input(self, packet):
        if packet.checksum == packet.seqnum + packet.acknum + int.from_bytes(packet.payload, byteorder='little'):
            #print("A: input packet check sum: ", packet.checksum)
            #print("base", self.base)
            #print("A: packet.acknum from input: ", packet.acknum)
            for i in range(self.base, self.base + self.window_size):
                #print("A: input i, length of the list: ", i, len(self.l))
                #print("packet acknum", packet.acknum)
                if i < len(self.l) and self.l[i].seqnum == packet.acknum:
                    #print("seqnum: ", self.l[i].seqnum)
                    self.base = i + 1
                    #print("A: next base: ", self.base)
                    break
            #print("A next seqnum", self.next_seqnum)
            #print("A window size + base", self.window_size + self.base)
            #print("list size", len(self.l))

            if self.base == self.make_seqnum:
                stop_timer(self)
            else:
                stop_timer(self)
                start_timer(self,self.timeout)

            while self.next_seqnum < self.base + self.window_size and self.next_seqnum < len(self.l)-1:
                to_layer3(self, self.l[self.next_seqnum])
                #print("A: sending from input: ", self.l[self.next_seqnum])
                self.next_seqnum = self.next_seqnum + 1
                #print("A: next seqnum number from input:", self.next_seqnum)
                if self.base == self.next_seqnum:
                    start_timer(self, self.timeout)
                ...
                #     #print("A: start_timer from list input")
    ...
    def timer_interrupt(self):
        ...
        for i in range(self.base, self.base + self.window_size):
            if i < len(self.l):
                to_layer3(self, self.l[i])
                #print("A: Resending", self.l[i])

        start_timer(self, self.timeout)
        #print("A: start_timer from interrupt")
        ...
        if packet.checksum == packet.seqnum + packet.acknum + int.from_bytes(packet.payload, byteorder='little'):
            if packet.seqnum == self.acknum:
                ...
                if self.acknum == self.seqnum_limit:
                    self.acknum = 0
        pkt = Pkt(0, (self.acknum - 1) % self.seqnum_limit, packet.acknum, b'00000000000000000000')
        pkt.checksum = pkt.acknum + int.from_bytes(pkt.payload, byteorder='little')
        to_layer3(self, pkt)
