...
class EntityA:
    ...
    def input(self, packet):
        if packet.checksum == packet.seqnum + packet.acknum + int.from_bytes(packet.payload, byteorder='little'):
            if self.next_seqnum == packet.acknum:
                stop_timer(self)
                self.next_seqnum += 1
                if self.next_seqnum == self.seqnum_limit:
                    self.next_seqnum = 0
                self.timerrunning = False
                self.takeFromQueue()
    ...
    def timer_interrupt(self):
        pkt = Pkt(self.next_seqnum, 0, self.currentSum, self.currentMessage.data)
        to_layer3(self, pkt)
        start_timer(self, 16)
    ...
    def takeFromQueue(self):
        if not self.q.empty() and not self.timerrunning:
            message = self.q.get()
            pkt = Pkt(self.next_seqnum, 0, 0, message.data)
            pkt.checksum = pkt.seqnum + pkt.acknum + int.from_bytes(pkt.payload, byteorder='little')
            self.currentSum = pkt.checksum
