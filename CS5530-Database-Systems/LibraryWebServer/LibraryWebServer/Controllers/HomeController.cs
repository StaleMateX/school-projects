...
namespace LibraryWebServer.Controllers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult CheckLogin( string name, int cardnum )
        {
            ...
            using (Team37LibraryContext db = new Team37LibraryContext())
            {
                var query = from p in db.Patrons
                            where p.Name == name && p.CardNum == cardnum
                            select p;

                // Check if any Patron matches the given name and card number
        ...
        public ActionResult AllTitles()
        {
            using (Team37LibraryContext db = new Team37LibraryContext())
            {
                var query =
                    ...
                    join c in db.CheckedOut on j.Serial equals c.Serial
                    into ck
                    from k in ck.DefaultIfEmpty()
                    join p in db.Patrons on k.CardNum equals p.CardNum
                    into pa
                    from n in pa.DefaultIfEmpty()
                    select new
                    {
                        isbn = t.Isbn,
                        title = t.Title,
        ...
        public ActionResult CheckOutBook( int serial )
        {
            ...
            uint serialNum = (uint)serial;
            uint cardNum = (uint)card;
            using (Team37LibraryContext db = new Team37LibraryContext())
            {
                // Create a new CheckedOut object
                CheckedOut ck = new CheckedOut
                {
                    CardNum = cardNum,
                    Serial = serialNum
                };

                // Add the new CheckedOut object to the database and save changes
                db.CheckedOut.Add(ck);
                db.SaveChanges();
            }
        ...
        public ActionResult ReturnBook( int serial )
        {
            ...
            {
                var query = from c in db.CheckedOut
                            where c.CardNum == card && c.Serial == serialNum
                            select c;

                CheckedOut ?ck = query.SingleOrDefault();

                if ( ck != null)
                {
                    // Remove the found CheckedOut record and save changes
                    db.CheckedOut.Remove(ck);
                    db.SaveChanges();
                ...
                {
                    // If the book is not found in the CheckedOut table, return false
                    return Json(new { success = false });
                }
            }
        }
