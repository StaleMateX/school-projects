...
namespace LMS.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        {
            // find all existing users with int
            int existingUserCount = db.Administrators.Count() + db.Professors.Count() + db.Students.Count();

            // increment the new user uID by 1
            int newUserID = existingUserCount + 1;

            // generate the uID
            string uID = "u" + newUserID.ToString("D7");

            // set the departmentAbbrev to the one that can be found or set it to null
            var departSubject = db.Departments.FirstOrDefault(d => d.Subject == departmentAbbrev);

            // add user into databases with different roles
            if( role == "Administrator")
            {
                db.Administrators.Add(new Administrator
                {
                    ...
                    FName = firstName,
                    LName = lastName,
                    Dob = new DateOnly(DOB.Year, DOB.Month, DOB.Day) //convert DateTime to DateOnly
                });
            }
            else if ( role == "Professor")
            {
                if(departSubject == null)
                {
                    throw new ArgumentException("Department not found or can not be null.");
                }
                else
                {
                    // department needed for Students and Professors
                    db.Professors.Add(new Professor
                    {
                        ...
                        LName = lastName,
                        Dob = new DateOnly(DOB.Year, DOB.Month, DOB.Day), //convert DateTime to DateOnly
                        DepartId = departSubject.DepartId
                    });
                }
            }
            ...
            {
                throw new ArgumentException("No role founded");
            }
